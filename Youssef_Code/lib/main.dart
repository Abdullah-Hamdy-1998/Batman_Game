...
class _startingState extends State<starting> with SingleTickerProviderStateMixin{
  ...
  _update(){
    ...
    for (Cloud cloud in clouds) {
      if (cloud.getRect(screensize, runDistance).right < 0) {
        setState(() {
          clouds.remove(cloud);
          clouds.add(Cloud(worldLocation: Offset(clouds.last.worldLocation.dx + Random().nextInt(100) + 50, Random().nextInt(40) - 20.0)));
        });
      }
    } //youssef_abdelrahman end
  ...
  Widget build(BuildContext context) {
    ...
    for (GameObject object in [...evil, bat, ...clouds, ...ground]){
      childern.add(
          AnimatedBuilder(
